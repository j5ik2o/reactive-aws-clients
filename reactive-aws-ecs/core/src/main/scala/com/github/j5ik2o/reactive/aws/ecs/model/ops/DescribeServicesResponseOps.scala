...
final class DescribeServicesResponseOps(val self: DescribeServicesResponse) extends AnyVal {
  ...
  @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
  final def servicesAsScala: Option[Seq[Service]] =
    Option(self.services).map { v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala }
...
trait ToDescribeServicesResponseOps {
  ...
  implicit def toDescribeServicesResponseBuilderOps(
      v: DescribeServicesResponse.Builder
  ): DescribeServicesResponseBuilderOps = new DescribeServicesResponseBuilderOps(v)

  implicit def toDescribeServicesResponseOps(v: DescribeServicesResponse): DescribeServicesResponseOps =
    new DescribeServicesResponseOps(v)
