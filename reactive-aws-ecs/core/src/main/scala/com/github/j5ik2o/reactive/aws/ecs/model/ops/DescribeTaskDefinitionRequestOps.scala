...
final class DescribeTaskDefinitionRequestOps(val self: DescribeTaskDefinitionRequest) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def includeAsScala: Option[Seq[TaskDefinitionField]] = Option(self.include).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toDescribeTaskDefinitionRequestBuilderOps(v: DescribeTaskDefinitionRequest.Builder): DescribeTaskDefinitionRequestBuilderOps = new DescribeTaskDefinitionRequestBuilderOps(v)

implicit def toDescribeTaskDefinitionRequestOps(v: DescribeTaskDefinitionRequest): DescribeTaskDefinitionRequestOps = new DescribeTaskDefinitionRequestOps(v)
