...
final class SetQueueAttributesRequestBuilderOps(val self: SetQueueAttributesRequest.Builder) extends AnyVal {
  ...
  final def withQueueUrlAsScala(value: Option[String]): SetQueueAttributesRequest.Builder = {
    value.fold(self) { v =>
      self.queueUrl(v)
    }
  } // String

  final def withAttributesAsScala(value: Option[Map[QueueAttributeName, String]]): SetQueueAttributesRequest.Builder = {
