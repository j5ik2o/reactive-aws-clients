...
final class SignalResourceRequestBuilderOps(val self: SignalResourceRequest.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            ...
            value.fold(self){ v => self.uniqueId(v) }
            } 
...
implicit def toSignalResourceRequestBuilderOps(v: SignalResourceRequest.Builder): SignalResourceRequestBuilderOps = new SignalResourceRequestBuilderOps(v)

implicit def toSignalResourceRequestOps(v: SignalResourceRequest): SignalResourceRequestOps = new SignalResourceRequestOps(v)
