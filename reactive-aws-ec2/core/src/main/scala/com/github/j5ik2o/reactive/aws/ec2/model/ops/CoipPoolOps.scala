...
final class CoipPoolBuilderOps(val self: CoipPool.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def poolCidrsAsScala(value: Option[Seq[String]]): CoipPool.Builder = {
                value.filter(_.nonEmpty).fold(self){ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.poolCidrs(v.asJava) } 
            }
...
final class CoipPoolOps(val self: CoipPool) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def poolCidrsAsScala: Option[Seq[String]] = Option(self.poolCidrs).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
