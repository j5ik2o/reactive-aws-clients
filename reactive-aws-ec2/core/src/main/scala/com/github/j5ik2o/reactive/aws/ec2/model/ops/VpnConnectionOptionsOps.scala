...
final class VpnConnectionOptionsOps(val self: VpnConnectionOptions) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def tunnelOptionsAsScala: Option[Seq[TunnelOption]] = Option(self.tunnelOptions).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toVpnConnectionOptionsBuilderOps(v: VpnConnectionOptions.Builder): VpnConnectionOptionsBuilderOps = new VpnConnectionOptionsBuilderOps(v)

implicit def toVpnConnectionOptionsOps(v: VpnConnectionOptions): VpnConnectionOptionsOps = new VpnConnectionOptionsOps(v)
