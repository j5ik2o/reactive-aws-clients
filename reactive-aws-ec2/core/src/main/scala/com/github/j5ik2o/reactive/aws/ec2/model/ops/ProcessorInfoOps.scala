...
final class ProcessorInfoBuilderOps(val self: ProcessorInfo.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def supportedArchitecturesAsScala(value: Option[Seq[ArchitectureType]]): ProcessorInfo.Builder = {
                value.filter(_.nonEmpty).fold(self){ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.supportedArchitectures(v.asJava) } 
            }
...
final class ProcessorInfoOps(val self: ProcessorInfo) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def supportedArchitecturesAsScala: Option[Seq[ArchitectureType]] = Option(self.supportedArchitectures).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toProcessorInfoBuilderOps(v: ProcessorInfo.Builder): ProcessorInfoBuilderOps = new ProcessorInfoBuilderOps(v)
