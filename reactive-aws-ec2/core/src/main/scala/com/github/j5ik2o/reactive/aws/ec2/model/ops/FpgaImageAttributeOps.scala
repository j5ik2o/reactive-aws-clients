...
final class FpgaImageAttributeBuilderOps(val self: FpgaImageAttribute.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def loadPermissionsAsScala(value: Option[Seq[LoadPermission]]): FpgaImageAttribute.Builder = {
                value.filter(_.nonEmpty).fold(self){ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.loadPermissions(v.asJava) } 
            }
...
final class FpgaImageAttributeOps(val self: FpgaImageAttribute) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def loadPermissionsAsScala: Option[Seq[LoadPermission]] = Option(self.loadPermissions).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toFpgaImageAttributeBuilderOps(v: FpgaImageAttribute.Builder): FpgaImageAttributeBuilderOps = new FpgaImageAttributeBuilderOps(v)
