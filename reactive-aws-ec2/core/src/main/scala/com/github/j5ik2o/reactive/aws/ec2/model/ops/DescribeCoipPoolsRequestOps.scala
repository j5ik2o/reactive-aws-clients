...
final class DescribeCoipPoolsRequestOps(val self: DescribeCoipPoolsRequest) extends AnyVal {
  ...
  @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
  final def poolIdsAsScala: Option[Seq[String]] =
    Option(self.poolIds).map { v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala }
...
trait ToDescribeCoipPoolsRequestOps {
  ...
  implicit def toDescribeCoipPoolsRequestBuilderOps(
      v: DescribeCoipPoolsRequest.Builder
  ): DescribeCoipPoolsRequestBuilderOps = new DescribeCoipPoolsRequestBuilderOps(v)

  implicit def toDescribeCoipPoolsRequestOps(v: DescribeCoipPoolsRequest): DescribeCoipPoolsRequestOps =
    new DescribeCoipPoolsRequestOps(v)
