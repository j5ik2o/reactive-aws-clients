...
final class Ipv6PoolBuilderOps(val self: Ipv6Pool.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def poolCidrBlocksAsScala(value: Option[Seq[PoolCidrBlock]]): Ipv6Pool.Builder = {
                value.filter(_.nonEmpty).fold(self){ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.poolCidrBlocks(v.asJava) } 
            }
...
final class Ipv6PoolOps(val self: Ipv6Pool) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def poolCidrBlocksAsScala: Option[Seq[PoolCidrBlock]] = Option(self.poolCidrBlocks).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
