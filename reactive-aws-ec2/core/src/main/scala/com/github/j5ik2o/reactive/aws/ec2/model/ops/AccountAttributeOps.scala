...
final class AccountAttributeBuilderOps(val self: AccountAttribute.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def attributeValuesAsScala(value: Option[Seq[AccountAttributeValue]]): AccountAttribute.Builder = {
                value.filter(_.nonEmpty).fold(self){ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.attributeValues(v.asJava) } 
            }
...
final class AccountAttributeOps(val self: AccountAttribute) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def attributeValuesAsScala: Option[Seq[AccountAttributeValue]] = Option(self.attributeValues).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toAccountAttributeBuilderOps(v: AccountAttribute.Builder): AccountAttributeBuilderOps = new AccountAttributeBuilderOps(v)
