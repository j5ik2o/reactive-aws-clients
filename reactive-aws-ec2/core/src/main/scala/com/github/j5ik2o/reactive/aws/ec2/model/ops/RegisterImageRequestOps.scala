...
final class RegisterImageRequestBuilderOps(val self: RegisterImageRequest.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def billingProductsAsScala(value: Option[Seq[String]]): RegisterImageRequest.Builder = {
                value.filter(_.nonEmpty).fold(self){ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.billingProducts(v.asJava) } 
            }
...
final class RegisterImageRequestOps(val self: RegisterImageRequest) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def billingProductsAsScala: Option[Seq[String]] = Option(self.billingProducts).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toRegisterImageRequestBuilderOps(v: RegisterImageRequest.Builder): RegisterImageRequestBuilderOps = new RegisterImageRequestBuilderOps(v)

implicit def toRegisterImageRequestOps(v: RegisterImageRequest): RegisterImageRequestOps = new RegisterImageRequestOps(v)
