...
final class InstanceStatusBuilderOps(val self: InstanceStatus.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def outpostArnAsScala(value: Option[String]): InstanceStatus.Builder = {
...
final class InstanceStatusOps(val self: InstanceStatus) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def eventsAsScala: Option[Seq[InstanceStatusEvent]] = Option(self.events).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toInstanceStatusBuilderOps(v: InstanceStatus.Builder): InstanceStatusBuilderOps = new InstanceStatusBuilderOps(v)
