...
final class ImportInstanceTaskDetailsOps(val self: ImportInstanceTaskDetails) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def volumesAsScala: Option[Seq[ImportInstanceVolumeDetailItem]] = Option(self.volumes).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toImportInstanceTaskDetailsBuilderOps(v: ImportInstanceTaskDetails.Builder): ImportInstanceTaskDetailsBuilderOps = new ImportInstanceTaskDetailsBuilderOps(v)

implicit def toImportInstanceTaskDetailsOps(v: ImportInstanceTaskDetails): ImportInstanceTaskDetailsOps = new ImportInstanceTaskDetailsOps(v)
