...
final class VCpuInfoBuilderOps(val self: VCpuInfo.Builder) extends AnyVal {
  ...
  final def validCoresAsScala(value: Option[Seq[Int]]): VCpuInfo.Builder = {
    value.filter(_.nonEmpty).map(_.map(_.asInstanceOf[java.lang.Integer])).fold(self) { v =>
      import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.validCores(v.asJava)
    }
  }
  ...
  final def validThreadsPerCoreAsScala(value: Option[Seq[Int]]): VCpuInfo.Builder = {
    value.filter(_.nonEmpty).map(_.map(_.asInstanceOf[java.lang.Integer])).fold(self) { v =>
      import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.validThreadsPerCore(v.asJava)
    }
  }
...
final class VCpuInfoOps(val self: VCpuInfo) extends AnyVal {
  ...
  @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
  final def validCoresAsScala: Option[Seq[Int]] = Option(self.validCores).map { v =>
    import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala.map(_.intValue())
  }

  @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
  final def validThreadsPerCoreAsScala: Option[Seq[Int]] = Option(self.validThreadsPerCore).map { v =>
    import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala.map(_.intValue())
  }
