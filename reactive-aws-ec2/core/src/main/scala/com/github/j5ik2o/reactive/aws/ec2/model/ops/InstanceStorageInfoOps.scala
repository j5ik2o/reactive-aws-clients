...
final class InstanceStorageInfoBuilderOps(val self: InstanceStorageInfo.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            ...
            value.fold(self){ v => self.totalSizeInGB(v) }
            } 

    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def disksAsScala(value: Option[Seq[DiskInfo]]): InstanceStorageInfo.Builder = {
                value.filter(_.nonEmpty).fold(self){ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.disks(v.asJava) } 
            }
...
final class InstanceStorageInfoOps(val self: InstanceStorageInfo) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def disksAsScala: Option[Seq[DiskInfo]] = Option(self.disks).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toInstanceStorageInfoBuilderOps(v: InstanceStorageInfo.Builder): InstanceStorageInfoBuilderOps = new InstanceStorageInfoBuilderOps(v)
