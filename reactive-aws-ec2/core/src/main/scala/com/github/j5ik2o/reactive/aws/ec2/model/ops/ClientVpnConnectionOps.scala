...
final class ClientVpnConnectionBuilderOps(val self: ClientVpnConnection.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            ...
            value.fold(self){ v => self.connectionEndTime(v) }
            } 
...
implicit def toClientVpnConnectionBuilderOps(v: ClientVpnConnection.Builder): ClientVpnConnectionBuilderOps = new ClientVpnConnectionBuilderOps(v)
