...
final class CertificateAuthenticationBuilderOps(val self: CertificateAuthentication.Builder) extends AnyVal {
  ...
  final def clientRootCertificateChainAsScala(value: Option[String]): CertificateAuthentication.Builder = {
    value.fold(self) { v =>
      self.clientRootCertificateChain(v)
    }
  }
...
trait ToCertificateAuthenticationOps {
  ...
  implicit def toCertificateAuthenticationBuilderOps(
      v: CertificateAuthentication.Builder
  ): CertificateAuthenticationBuilderOps = new CertificateAuthenticationBuilderOps(v)

  implicit def toCertificateAuthenticationOps(v: CertificateAuthentication): CertificateAuthenticationOps =
    new CertificateAuthenticationOps(v)
