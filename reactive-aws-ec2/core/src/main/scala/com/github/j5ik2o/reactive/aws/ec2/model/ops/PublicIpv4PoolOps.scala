...
final class PublicIpv4PoolBuilderOps(val self: PublicIpv4Pool.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def poolAddressRangesAsScala(value: Option[Seq[PublicIpv4PoolRange]]): PublicIpv4Pool.Builder = {
                value.filter(_.nonEmpty).fold(self){ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.poolAddressRanges(v.asJava) } 
            }
...
final class PublicIpv4PoolOps(val self: PublicIpv4Pool) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def poolAddressRangesAsScala: Option[Seq[PublicIpv4PoolRange]] = Option(self.poolAddressRanges).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toPublicIpv4PoolBuilderOps(v: PublicIpv4Pool.Builder): PublicIpv4PoolBuilderOps = new PublicIpv4PoolBuilderOps(v)
