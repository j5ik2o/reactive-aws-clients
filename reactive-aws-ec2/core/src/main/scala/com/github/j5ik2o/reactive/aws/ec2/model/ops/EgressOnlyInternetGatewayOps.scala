...
final class EgressOnlyInternetGatewayOps(val self: EgressOnlyInternetGateway) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def attachmentsAsScala: Option[Seq[InternetGatewayAttachment]] = Option(self.attachments).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toEgressOnlyInternetGatewayBuilderOps(v: EgressOnlyInternetGateway.Builder): EgressOnlyInternetGatewayBuilderOps = new EgressOnlyInternetGatewayBuilderOps(v)

implicit def toEgressOnlyInternetGatewayOps(v: EgressOnlyInternetGateway): EgressOnlyInternetGatewayOps = new EgressOnlyInternetGatewayOps(v)
