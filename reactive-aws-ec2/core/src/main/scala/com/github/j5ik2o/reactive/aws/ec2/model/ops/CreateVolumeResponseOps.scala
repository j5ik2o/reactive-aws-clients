...
final class CreateVolumeResponseOps(val self: CreateVolumeResponse) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def attachmentsAsScala: Option[Seq[VolumeAttachment]] = Option(self.attachments).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toCreateVolumeResponseBuilderOps(v: CreateVolumeResponse.Builder): CreateVolumeResponseBuilderOps = new CreateVolumeResponseBuilderOps(v)

implicit def toCreateVolumeResponseOps(v: CreateVolumeResponse): CreateVolumeResponseOps = new CreateVolumeResponseOps(v)
