...
final class OnDemandOptionsOps(val self: OnDemandOptions) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def capacityReservationOptionsAsScala: Option[CapacityReservationOptions] = Option(self.capacityReservationOptions) 
...
implicit def toOnDemandOptionsBuilderOps(v: OnDemandOptions.Builder): OnDemandOptionsBuilderOps = new OnDemandOptionsBuilderOps(v)
