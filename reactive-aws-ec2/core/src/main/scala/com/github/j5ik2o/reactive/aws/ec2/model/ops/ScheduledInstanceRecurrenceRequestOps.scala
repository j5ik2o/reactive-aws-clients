...
final class ScheduledInstanceRecurrenceRequestBuilderOps(val self: ScheduledInstanceRecurrenceRequest.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def occurrenceDaysAsScala(value: Option[Seq[Int]]): ScheduledInstanceRecurrenceRequest.Builder = {
                value.filter(_.nonEmpty).map(_.map(_.asInstanceOf[java.lang.Integer])).fold(self){ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.occurrenceDays(v.asJava) } 
            }
...
final class ScheduledInstanceRecurrenceRequestOps(val self: ScheduledInstanceRecurrenceRequest) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def occurrenceDaysAsScala: Option[Seq[Int]] = Option(self.occurrenceDays).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala.map(_.intValue()) } 
...
implicit def toScheduledInstanceRecurrenceRequestBuilderOps(v: ScheduledInstanceRecurrenceRequest.Builder): ScheduledInstanceRecurrenceRequestBuilderOps = new ScheduledInstanceRecurrenceRequestBuilderOps(v)

implicit def toScheduledInstanceRecurrenceRequestOps(v: ScheduledInstanceRecurrenceRequest): ScheduledInstanceRecurrenceRequestOps = new ScheduledInstanceRecurrenceRequestOps(v)
