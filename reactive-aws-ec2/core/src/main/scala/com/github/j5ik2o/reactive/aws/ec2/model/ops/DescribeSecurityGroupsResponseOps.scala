...
final class DescribeSecurityGroupsResponseOps(val self: DescribeSecurityGroupsResponse) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def securityGroupsAsScala: Option[Seq[SecurityGroup]] = Option(self.securityGroups).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toDescribeSecurityGroupsResponseBuilderOps(v: DescribeSecurityGroupsResponse.Builder): DescribeSecurityGroupsResponseBuilderOps = new DescribeSecurityGroupsResponseBuilderOps(v)

implicit def toDescribeSecurityGroupsResponseOps(v: DescribeSecurityGroupsResponse): DescribeSecurityGroupsResponseOps = new DescribeSecurityGroupsResponseOps(v)
