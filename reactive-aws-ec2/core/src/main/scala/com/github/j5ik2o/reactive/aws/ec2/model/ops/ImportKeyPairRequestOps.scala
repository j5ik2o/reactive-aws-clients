...
final class ImportKeyPairRequestBuilderOps(val self: ImportKeyPairRequest.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def publicKeyMaterialAsScala(value: Option[software.amazon.awssdk.core.SdkBytes]): ImportKeyPairRequest.Builder = {
            value.fold(self){ v => self.publicKeyMaterial(v) }
            } 
...
implicit def toImportKeyPairRequestBuilderOps(v: ImportKeyPairRequest.Builder): ImportKeyPairRequestBuilderOps = new ImportKeyPairRequestBuilderOps(v)

implicit def toImportKeyPairRequestOps(v: ImportKeyPairRequest): ImportKeyPairRequestOps = new ImportKeyPairRequestOps(v)
