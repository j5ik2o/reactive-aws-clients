...
final class DescribeTagsResponseOps(val self: DescribeTagsResponse) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def tagsAsScala: Option[Seq[TagDescription]] = Option(self.tags).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toDescribeTagsResponseBuilderOps(v: DescribeTagsResponse.Builder): DescribeTagsResponseBuilderOps = new DescribeTagsResponseBuilderOps(v)

implicit def toDescribeTagsResponseOps(v: DescribeTagsResponse): DescribeTagsResponseOps = new DescribeTagsResponseOps(v)
