...
final class FpgaInfoBuilderOps(val self: FpgaInfo.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def fpgasAsScala(value: Option[Seq[FpgaDeviceInfo]]): FpgaInfo.Builder = {
                value.filter(_.nonEmpty).fold(self){ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.fpgas(v.asJava) } 
            }
...
final class FpgaInfoOps(val self: FpgaInfo) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def fpgasAsScala: Option[Seq[FpgaDeviceInfo]] = Option(self.fpgas).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
