...
final class ReservationValueBuilderOps(val self: ReservationValue.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            ...
            value.fold(self){ v => self.remainingTotalValue(v) }
            } 

    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def remainingUpfrontValueAsScala(value: Option[String]): ReservationValue.Builder = {
            value.fold(self){ v => self.remainingUpfrontValue(v) }
            } 
...
implicit def toReservationValueBuilderOps(v: ReservationValue.Builder): ReservationValueBuilderOps = new ReservationValueBuilderOps(v)
