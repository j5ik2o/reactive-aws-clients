...
final class ScheduledInstanceRecurrenceBuilderOps(val self: ScheduledInstanceRecurrence.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def occurrenceDaySetAsScala(value: Option[Seq[Int]]): ScheduledInstanceRecurrence.Builder = {
                value.filter(_.nonEmpty).map(_.map(_.asInstanceOf[java.lang.Integer])).fold(self){ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.occurrenceDaySet(v.asJava) } 
            }
...
final class ScheduledInstanceRecurrenceOps(val self: ScheduledInstanceRecurrence) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def occurrenceDaySetAsScala: Option[Seq[Int]] = Option(self.occurrenceDaySet).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala.map(_.intValue()) } 
...
implicit def toScheduledInstanceRecurrenceBuilderOps(v: ScheduledInstanceRecurrence.Builder): ScheduledInstanceRecurrenceBuilderOps = new ScheduledInstanceRecurrenceBuilderOps(v)

implicit def toScheduledInstanceRecurrenceOps(v: ScheduledInstanceRecurrence): ScheduledInstanceRecurrenceOps = new ScheduledInstanceRecurrenceOps(v)
