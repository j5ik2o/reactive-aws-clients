...
final class LaunchTemplateBuilderOps(val self: LaunchTemplate.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            ...
            value.fold(self){ v => self.defaultVersionNumber(v) }
            } 

    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def latestVersionNumberAsScala(value: Option[Long]): LaunchTemplate.Builder = {
            value.fold(self){ v => self.latestVersionNumber(v) }
            } 
...
implicit def toLaunchTemplateBuilderOps(v: LaunchTemplate.Builder): LaunchTemplateBuilderOps = new LaunchTemplateBuilderOps(v)
