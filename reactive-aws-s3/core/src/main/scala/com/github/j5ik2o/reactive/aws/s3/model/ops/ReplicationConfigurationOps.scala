...
final class ReplicationConfigurationOps(val self: ReplicationConfiguration) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def rulesAsScala: Option[Seq[ReplicationRule]] = Option(self.rules).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toReplicationConfigurationBuilderOps(v: ReplicationConfiguration.Builder): ReplicationConfigurationBuilderOps = new ReplicationConfigurationBuilderOps(v)

implicit def toReplicationConfigurationOps(v: ReplicationConfiguration): ReplicationConfigurationOps = new ReplicationConfigurationOps(v)
