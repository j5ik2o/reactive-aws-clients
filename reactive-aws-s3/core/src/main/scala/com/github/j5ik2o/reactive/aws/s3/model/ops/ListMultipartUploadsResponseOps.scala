...
final class ListMultipartUploadsResponseBuilderOps(val self: ListMultipartUploadsResponse.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def uploadsAsScala(value: Option[Seq[MultipartUpload]]): ListMultipartUploadsResponse.Builder = {
                value.filter(_.nonEmpty).fold(self){ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.uploads(v.asJava) } 
            }
...
final class ListMultipartUploadsResponseOps(val self: ListMultipartUploadsResponse) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def uploadsAsScala: Option[Seq[MultipartUpload]] = Option(self.uploads).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toListMultipartUploadsResponseBuilderOps(v: ListMultipartUploadsResponse.Builder): ListMultipartUploadsResponseBuilderOps = new ListMultipartUploadsResponseBuilderOps(v)

implicit def toListMultipartUploadsResponseOps(v: ListMultipartUploadsResponse): ListMultipartUploadsResponseOps = new ListMultipartUploadsResponseOps(v)
