...
final class RestoreObjectResponseBuilderOps(val self: RestoreObjectResponse.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            ...
            value.fold(self){ v => self.restoreOutputPath(v) }
            } 
...
implicit def toRestoreObjectResponseBuilderOps(v: RestoreObjectResponse.Builder): RestoreObjectResponseBuilderOps = new RestoreObjectResponseBuilderOps(v)

implicit def toRestoreObjectResponseOps(v: RestoreObjectResponse): RestoreObjectResponseOps = new RestoreObjectResponseOps(v)
