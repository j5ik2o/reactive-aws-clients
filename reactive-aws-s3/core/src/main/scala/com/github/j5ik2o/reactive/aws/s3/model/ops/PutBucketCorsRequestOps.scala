...
final class PutBucketCorsRequestBuilderOps(val self: PutBucketCorsRequest.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            ...
            value.fold(self){ v => self.corsConfiguration(v) }
            } 
...
implicit def toPutBucketCorsRequestBuilderOps(v: PutBucketCorsRequest.Builder): PutBucketCorsRequestBuilderOps = new PutBucketCorsRequestBuilderOps(v)

implicit def toPutBucketCorsRequestOps(v: PutBucketCorsRequest): PutBucketCorsRequestOps = new PutBucketCorsRequestOps(v)
