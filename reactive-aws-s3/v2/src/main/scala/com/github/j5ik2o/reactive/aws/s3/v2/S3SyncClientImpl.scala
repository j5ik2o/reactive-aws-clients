...
private[s3] class S3SyncClientImpl(override val underlying: JavaS3Client) extends S3SyncClient {
  ...
  override def getObject(getObjectRequest: GetObjectRequest): Either[Throwable, ResponseInputStream] = {
    import GetObjectRequestOps._
    import ResponseInputStreamOps._
    toEither(underlying.getObject(getObjectRequest.toJava)).right.map(_.toScala)
  }

  override def getObjectAsBytes(getObjectRequest: GetObjectRequest): Either[Throwable, ResponseBytes] = {
    import GetObjectRequestOps._
    import ResponseBytesOps._
    toEither(underlying.getObjectAsBytes(getObjectRequest.toJava)).right.map(_.toScala)
  }
  ...
  ): Either[Throwable, ResponseInputStream] = {
    import GetObjectTorrentRequestOps._
    import ResponseInputStreamOps._
    toEither(underlying.getObjectTorrent(getObjectTorrentRequest.toJava)).right.map(_.toScala)
  }

  override def getObjectTorrentAsBytes(
      getObjectTorrentRequest: GetObjectTorrentRequest
  ): Either[Throwable, ResponseBytes] = {
    import GetObjectTorrentRequestOps._
    import ResponseBytesOps._
    toEither(underlying.getObjectTorrentAsBytes(getObjectTorrentRequest.toJava)).right.map(_.toScala)
  }
  ...
  override def listPartsPaginator(listPartsRequest: ListPartsRequest): ListPartsIterable = {
    import ListPartsRequestOps._
