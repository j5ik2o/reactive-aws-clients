...
lazy val `reactive-aws-dynamodb-v2-akka` = (project in file("reactive-aws-dynamodb/v2-akka")).settings(
  coreSettings ++ Seq(
    ...
    templateNameMapper in scalaWrapperGen := {
      case ("DynamoDBStreamClient", cd: ClassDesc) if cd.simpleTypeName == "DynamoDbAsyncClient" =>
        "DynamoDBStreamClient.ftl"
      case ("DynamoDBStreamClientImpl", cd: ClassDesc) if cd.simpleTypeName == "DynamoDbAsyncClient" =>
        "DynamoDBStreamClientImpl.ftl"
      case (name, cd) => throw new Exception(s"error: ${name}, ${cd.simpleTypeName}")
    },
    typeNameMapper in scalaWrapperGen := {
      case cd if cd.simpleTypeName == "DynamoDbAsyncClient" =>
        Seq("DynamoDBStreamClient", "DynamoDBStreamClientImpl")
    },
    packageNameMapper in scalaWrapperGen := {
...
lazy val `reactive-aws-kinesis-core` = (project in file("reactive-aws-kinesis/core")).settings(
  coreSettings ++ Seq(
    ...
    // compile in Compile := ((compile in Compile) dependsOn (generateAll in scalaWrapperGen)).value,
    templateNameMapper in scalaWrapperGen := {
      case (_, cd) if cd.simpleTypeName == "KinesisAsyncClient" => "KinesisClient.ftl"
      case (_, _: EnumDesc)                                     => "EnumModel.ftl"
      case (_, cd: ClassDesc)
          if cd.simpleTypeName.endsWith("Response") && cd.packageName.exists(_.endsWith("model")) && !cd.isStatic =>
        "ResponseModel.ftl"
