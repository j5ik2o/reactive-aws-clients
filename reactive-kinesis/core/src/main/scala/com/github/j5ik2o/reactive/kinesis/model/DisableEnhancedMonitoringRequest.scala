...
final case class DisableEnhancedMonitoringRequest(streamName: Option[String] = None,
                                                  shardLevelMetrics: Option[Seq[MetricsName]] = None) {
  def withStreamName(value: Option[String]): DisableEnhancedMonitoringRequest = copy(streamName = value)
