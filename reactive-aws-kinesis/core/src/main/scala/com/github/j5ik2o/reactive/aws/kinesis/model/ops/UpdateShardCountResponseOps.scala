...
final class UpdateShardCountResponseBuilderOps(val self: UpdateShardCountResponse.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            ...
            value.fold(self){ v => self.currentShardCount(v) }
            } 
...
implicit def toUpdateShardCountResponseBuilderOps(v: UpdateShardCountResponse.Builder): UpdateShardCountResponseBuilderOps = new UpdateShardCountResponseBuilderOps(v)

implicit def toUpdateShardCountResponseOps(v: UpdateShardCountResponse): UpdateShardCountResponseOps = new UpdateShardCountResponseOps(v)
