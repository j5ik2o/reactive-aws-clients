...
final class SubscribeToShardEventBuilderOps(val self: SubscribeToShardEvent.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def recordsAsScala(value: Option[Seq[Record]]): SubscribeToShardEvent.Builder = {
                value.filter(_.nonEmpty).fold(self){ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.records(v.asJava) } 
            }
...
final class SubscribeToShardEventOps(val self: SubscribeToShardEvent) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def recordsAsScala: Option[Seq[Record]] = Option(self.records).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala } 
...
implicit def toSubscribeToShardEventBuilderOps(v: SubscribeToShardEvent.Builder): SubscribeToShardEventBuilderOps = new SubscribeToShardEventBuilderOps(v)

implicit def toSubscribeToShardEventOps(v: SubscribeToShardEvent): SubscribeToShardEventOps = new SubscribeToShardEventOps(v)
