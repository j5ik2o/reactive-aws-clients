...
final class TableAutoScalingDescriptionOps(val self: TableAutoScalingDescription) extends AnyVal {
  ...
  @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
  final def replicasAsScala: Option[Seq[ReplicaAutoScalingDescription]] =
    Option(self.replicas).map { v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala }
...
trait ToTableAutoScalingDescriptionOps {
  ...
  implicit def toTableAutoScalingDescriptionBuilderOps(
      v: TableAutoScalingDescription.Builder
  ): TableAutoScalingDescriptionBuilderOps = new TableAutoScalingDescriptionBuilderOps(v)

  implicit def toTableAutoScalingDescriptionOps(v: TableAutoScalingDescription): TableAutoScalingDescriptionOps =
    new TableAutoScalingDescriptionOps(v)
