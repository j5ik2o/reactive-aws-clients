...
final case class BackupDescription(
  ...
  def withBackupDetails(value: Option[BackupDetails]): BackupDescription =
    copy(backupDetails = value)
  def withSourceTableDetails(value: Option[SourceTableDetails]): BackupDescription =
    copy(sourceTableDetails = value)
