...
final class QueryResponseBuilderOps(val self: QueryResponse.Builder) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def itemsAsScala(value: Option[Seq[Map[String, AttributeValue]]]): QueryResponse.Builder = {
                    value.filter(_.nonEmpty).fold(self){ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.items(v.map(_.asJava).asJava) } 
            }
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
            final def lastEvaluatedKeyAsScala(value: Option[Map[String, AttributeValue]]): QueryResponse.Builder = {
                value.filter(_.nonEmpty).fold(self){ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; self.lastEvaluatedKey(v.asJava) } 
            }
...
final class QueryResponseOps(val self: QueryResponse) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def itemsAsScala: Option[Seq[Map[String, AttributeValue]]] = Option(self.items).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala.map(_.asScala.toMap) } 
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def lastEvaluatedKeyAsScala: Option[Map[String, AttributeValue]]  = Option(self.lastEvaluatedKey).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala.toMap } 
...
implicit def toQueryResponseBuilderOps(v: QueryResponse.Builder): QueryResponseBuilderOps = new QueryResponseBuilderOps(v)
