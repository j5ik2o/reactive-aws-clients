...
final case class UpdateGlobalSecondaryIndexAction(indexName: Option[String] = None,
                                                  provisionedThroughput: Option[ProvisionedThroughput] = None) {
  def withIndexName(value: Option[String]): UpdateGlobalSecondaryIndexAction = copy(indexName = value)
