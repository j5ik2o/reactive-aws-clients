...
final class ConsumedCapacityOps(val self: ConsumedCapacity) extends AnyVal {
    ...
    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def localSecondaryIndexesAsScala: Option[Map[String, Capacity]]  = Option(self.localSecondaryIndexes).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala.toMap } 

    @SuppressWarnings(Array("org.wartremover.warts.AsInstanceOf"))
                final def globalSecondaryIndexesAsScala: Option[Map[String, Capacity]]  = Option(self.globalSecondaryIndexes).map{ v => import com.github.j5ik2o.reactive.aws.utils.JavaCollectionHelper._; v.asScala.toMap } 
...
implicit def toConsumedCapacityBuilderOps(v: ConsumedCapacity.Builder): ConsumedCapacityBuilderOps = new ConsumedCapacityBuilderOps(v)
