  ...
  implicit class ScalaTransactWriteItemsRequestOps(val self: ScalaTransactWriteItemsRequest) extends AnyVal {
    ...
    def toJava: JavaTransactWriteItemsRequest = {
      val result = JavaTransactWriteItemsRequest.builder()
                                                self.transactItems.filter(_.nonEmpty).foreach{ v => import scala.collection.JavaConverters._, TransactWriteItemOps._; result.transactItems(v.map(_.toJava).asJava) } // Seq[TransactWriteItem], case Seq[_], UserDefined
