...
import com.github.j5ik2o.reactive.aws.dynamodb.model.{ AutoScalingSettingsDescription => ScalaAutoScalingSettingsDescription, _ }
import software.amazon.awssdk.services.dynamodb.model.{ AutoScalingSettingsDescription => JavaAutoScalingSettingsDescription }
    ...
    def toJava: JavaAutoScalingSettingsDescription = {
      val result = JavaAutoScalingSettingsDescription.builder()
                      ...
                      self.autoScalingRoleArn.filter(_.nonEmpty).foreach(v => result.autoScalingRoleArn(v)) // String, case String
                              self.scalingPolicies.filter(_.nonEmpty).foreach{ v => import scala.collection.JavaConverters._, AutoScalingPolicyDescriptionOps._; result.scalingPolicies(v.map(_.toJava).asJava) } // Seq[AutoScalingPolicyDescription], case Seq[_], UserDefined
     ...
     def toScala: ScalaAutoScalingSettingsDescription = {
       ScalaAutoScalingSettingsDescription()
            ...
            .withAutoScalingRoleArn(Option(self.autoScalingRoleArn)) // String
                    .withScalingPolicies(Option(self.scalingPolicies).map{ v => import scala.collection.JavaConverters._, AutoScalingPolicyDescriptionOps._; v.asScala.map(_.toScala)}) // Seq[AutoScalingPolicyDescription], Seq-6
     }
