    ...
    def toScala: ScalaListBackupsResponse = {
      ScalaListBackupsResponse()
        ...
        .withHttpHeaders(Option(self.sdkHttpResponse().headers().asScala.mapValues(_.asScala).toMap))
                    .withBackupSummaries(Option(self.backupSummaries).map{ v => import scala.collection.JavaConverters._, BackupSummaryOps._; v.asScala.map(_.toScala)}) // Seq[BackupSummary], Seq-6
