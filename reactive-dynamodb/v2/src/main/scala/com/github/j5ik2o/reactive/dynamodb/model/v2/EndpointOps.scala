...
import com.github.j5ik2o.reactive.dynamodb.model.{ Endpoint => ScalaEndpoint }
...
object EndpointOps {
  ...
  implicit class JavaEndpointOps(val self: JavaEndpoint) extends AnyVal {
    ...
    def toScala: ScalaEndpoint = {
      ScalaEndpoint()
        .withAddress(Option(self.address)).withCachePeriodInMinutes(Option(self.cachePeriodInMinutes))
    }
