...
import com.github.j5ik2o.reactive.dynamodb.model.{ TimeToLiveSpecification => ScalaTimeToLiveSpecification }
  ...
  implicit class JavaTimeToLiveSpecificationOps(val self: JavaTimeToLiveSpecification) extends AnyVal {
    ...
    def toScala: ScalaTimeToLiveSpecification = {
      ScalaTimeToLiveSpecification()
        .withAttributeName(Option(self.attributeName)).withEnabled(Option(self.enabled).map(_.booleanValue()))
    }
