...
final case class UpdateTimeToLiveRequest(tableName: Option[String] = None,
                                         timeToLiveSpecification: Option[TimeToLiveSpecification] = None) {
  def withTableName(value: Option[String]): UpdateTimeToLiveRequest = copy(tableName = value)
