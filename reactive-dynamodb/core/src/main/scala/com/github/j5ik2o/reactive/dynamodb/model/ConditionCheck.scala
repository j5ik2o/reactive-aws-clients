...
final case class ConditionCheck(
  ...
  def withKey(value: Option[Map[String, AttributeValue]]): ConditionCheck = copy(key = value)
  def withTableName(value: Option[String]): ConditionCheck                = copy(tableName = value)
  def withConditionExpression(value: Option[String]): ConditionCheck      = copy(conditionExpression = value)
